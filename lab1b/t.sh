...
./$SERVER --port=$PORT --shell=$SHELL > SVR_OUT 2> SVR_ERR &
...
if [ -s LOG_RECV_1 ]; then
    ...
    grep "$SHELL" LOG_RECV_1 > /dev/null
    if [ $? -eq 0 ]; then
...
let PORT+=1
./$SERVER --port=$PORT --compress --shell=$SHELL > SVR_OUT 2> SVR_ERR &
...
grep "$SHELL" LOG_RECV_2 > /dev/null
if [ $? -eq 0 ]; then
